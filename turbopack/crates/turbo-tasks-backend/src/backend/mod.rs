...
impl<B: BackingStorage> TurboTasksBackendInner<B> {
    ...
    fn task_execution_completed(
        ...
        // Connect children
        {
            ...
            children_diff += new_children.len() as i32;

            // Update the children count
            if children_diff > 0 {
                update_ucount_and_get!(task, ChildrenCount, children_diff as u32);
            } else {
                let update = (-children_diff) as u32;
                update_ucount_and_get!(task, ChildrenCount, -update);
            }
